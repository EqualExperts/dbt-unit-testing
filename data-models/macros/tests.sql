{% macro test(model_name, test_description) %}
    ...
    {% for k, v in test_info_json.items() %}
      {% set dummy = test_info_json.update({k: sql_decode(v)}) %}
    {% endfor %}
    
    {% set expectations = test_info_json['__EXPECTATIONS__'] %}
    {% set dummy = test_info_json.pop('__EXPECTATIONS__') %}

    {{ run_test(model_name, test_description, test_info_json, expectations)}}
{% endmacro %}
...
{% macro mock_ref(model_name) %}
    {{ mock_input(model_name, '', caller()) }}
{% endmacro %}

{% macro mock_source(source_name, model_name) %}
    {{ mock_input(model_name, source_name, caller()) }}
{% endmacro %}
...
{% macro mock_input(model_name, source_name, input_values_sql) %}
    ...
    {% set model_sql %}
      {%- set extra_columns = extract_columns_difference('select * from ' + model + ' where false', input_values_sql) -%}
    ...
    {%- set input_as_json = '"' + model_name + '": "' + sql_encode(model_sql) + '",' -%}
...
{% macro expect() %}
    ...
    {%- set input_as_json = '"__EXPECTATIONS__": "' + sql_encode(model_sql) + '",' -%}
    {% do return (input_as_json) %}
{% endmacro %}
...
{% macro run_test(model_name, test_description, test_inputs, expectations) %}
  ...
  {%- set test_sql -%}
    ...
    actual as (select {{columns}}, count(*) as count from ( {{ build_model_complete_sql(model_name, test_inputs_models) }} ) as s group by {{columns}}),
