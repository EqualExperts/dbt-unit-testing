{% macro print_table(agate_table) %}
  {% set columns_info = [] %}
  {% for col_name in agate_table.column_names %}
    {% set column_info = {"max_length": col_name | length, "is_string": false} %}
    {% for row in agate_table.rows %}
      {% set cell_value = row[col_name] %}
      {% set l = cell_value | string | length %}
      {% if l > column_info.max_length %}
        {% do column_info.update({"max_length": l}) %}
      {% endif %}
    {% do column_info.update({"is_string": column_info.is_string or cell_value is string}) %}
    {% endfor %}
    {% set columns_info = columns_info.append(column_info) %}
  {% endfor %}

  {% set cells = [] %}
  {% for col_name in agate_table.column_names %}
    {% set padded = dbt_unit_testing.pad(col_name, columns_info[loop.index0].max_length, pad_right=columns_info[loop.index0].is_string) %}
    {% do cells.append(padded) %}
  {% endfor %}
  {% do log("| " ~ cells | join(" | ") ~ " |", info=True) %}

  {% set cells = [] %}
  {% for col_name in agate_table.column_names %}
    {% do cells.append(dbt_unit_testing.pad("", columns_info[loop.index0].max_length, c="-")) %}
  {% endfor %}
  {% do log("| " ~ cells | join(" | ") ~ " |", info=True) %}

  {% for row in agate_table.rows %}
    {% set cells = [] %}
    {% for cell_value in row %}
      {% set padded = dbt_unit_testing.pad(cell_value, columns_info[loop.index0].max_length, pad_right=cell_value is string) %}
      {% do cells.append(padded) %}
    {% endfor %}
    {% do log("| " ~ cells | join(" | ") ~ " |", info=True) %}
  {% endfor %}
{% endmacro %}

{% macro pad(v, pad, pad_right=false, c=" ") %}
  {% set padding = c * (pad - v | string | length) %}
  {% if pad_right %}
    {{ return (v ~ padding) }}
  {% else %}
    {{ return (padding ~ v) }}
  {% endif %}
{% endmacro %}

{% macro parse_colors(s) %}
  {{ return (s
      .replace("{RED}", "\x1b[0m\x1b[31m")
      .replace("{GREEN}", "\x1b[0m\x1b[32m")
      .replace("{YELLOW}", "\x1b[0m\x1b[33m")
      .replace("{RESET}", "\x1b[0m")) }}
{% endmacro %}

{% macro println(s) %}
  {% do log(dbt_unit_testing.parse_colors(s ~ "{RESET}"), info=true) %}
{% endmacro %}
